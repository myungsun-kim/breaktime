...
public class UserController {
	...
	@GetMapping("/{userId}")
	@ApiOperation(value = "존재하는 회원 확인용", notes = "로그인한 사용자가 아닌 경우에만 응답")
	@ApiResponses({
	        @ApiResponse(code = 200, message = "존재하는 사용자 없음"),
	...
	public ResponseEntity<? extends BaseResponseBody> sss (@PathVariable("userId") String userId ) {
		...
		return ResponseEntity.status(409).body(BaseResponseBody.of(200, "사용자 ID 없음")); 
